...
#            break  # 遇到已读就停止
    ...
    # ✅ 下载并重命名为 Z.yaml
    save_path = await last_yaml_msg.download_media(file="Z.yaml")
    ...
    # ✅ 取【倒数第三个】yaml 文件
    last_yaml_msg = unread_yaml_files[2]
    print(f"\n🎉 找到倒数第三个 yaml 文件：{last_yaml_msg.file.name}")
    
    # ✅ 下载并重命名为 X.yaml
    save_path = await last_yaml_msg.download_media(file="X.yaml")
    print(f"✅ 下载完成！保存路径：{save_path}")
